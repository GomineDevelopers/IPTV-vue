...
export default {
  ...
  props: ["api_data_m3", "api_data_m3_range"],
  computed: {
  ...
  watch: {
    ...
    api_data_m3(newValue, oldValue) {
      // console.log("api_data_m3 - 联通专项数据", newValue)
      let vm = this;
      ...
      try {
        ...
        vm.GT_UVWR1_S2.content[0].data = register_num_temp;
        vm.GT_UVWR1_S2.content[1].data = new_num_temp;
      } catch (error) {
        vm.GT_UVWR1_S1.data = [["product", "在册用户数", "新增在册用户数"], [], [], [], [], [], [], [], [], []]
        vm.GT_UVWR1_S2.content[0].data = []
        vm.GT_UVWR1_S2.content[1].data = []
        console.log(error);
      }
      ...
      try {
        ...
        //一周总体观看数据
        let onlive1 =
          newValue.data.responses[3].aggregations.onlive_user_num.value;
        ...
        let demand1 =
          newValue.data.responses[4].aggregations.demand_user_num.value;
        ...
        let watch1 =
          newValue.data.responses[5].aggregations.watch_user_num.value;
      ...
      try {
        ...
        let open_num =
          newValue.data.responses[6].aggregations.statistical_granularity
            .buckets; //一周开机用户数
        let onlive_num =
          newValue.data.responses[7].aggregations.statistical_granularity
            .buckets; //一周直播观看用户
        let demand_num =
          newValue.data.responses[8].aggregations.statistical_granularity
            .buckets; //一周点播观看用户
        let watch_num =
          newValue.data.responses[9].aggregations.statistical_granularity
            .buckets; //一周回看观看用户
        ...
        vm.GT_UVWR1_T2.data = week_user_temp;
      } catch (error) {
        vm.GT_UVWR1_T2.data = []
        console.log(error);
      }
      ...
      try {
        ...
        Vue.set(vm.GT_UVWR1_U1.data[0], 1, beforeWeekFormat);
        Vue.set(vm.GT_UVWR1_U1.data[0], 2, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_U2.data[0], 1, beforeWeekFormat);
        Vue.set(vm.GT_UVWR1_U2.data[0], 2, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_U3.data[0], 1, beforeWeekFormat);
        Vue.set(vm.GT_UVWR1_U3.data[0], 2, currentWeekFormat);
        if (onlive_data[0].key) { }
        onlive_data.forEach((value, index) => {
          switch (value.key) {
            case "央视":
              Vue.set(
                vm.GT_UVWR1_U1.data[1],
                2,
                Number((value.onlive_user_num.value / 10000).toFixed(1))
              );
              Vue.set(
                vm.GT_UVWR1_U2.data[1],
                2,
                Number((value.onlive_freq.value / 10000).toFixed(1))
              );
              Vue.set(
                vm.GT_UVWR1_U3.data[1],
                2,
                Number((value.onlive_dur.value / 60).toFixed(1))
              );
              break;
            case "卫视":
              Vue.set(
                vm.GT_UVWR1_U1.data[2],
                2,
                Number((value.onlive_user_num.value / 10000).toFixed(1))
              );
              Vue.set(
                vm.GT_UVWR1_U2.data[2],
                2,
                Number((value.onlive_freq.value / 10000).toFixed(1))
              );
              Vue.set(
            ...
            case "本地":
              Vue.set(
                vm.GT_UVWR1_U1.data[3],
                2,
                Number((value.onlive_user_num.value / 10000).toFixed(1))
              );
              Vue.set(
                vm.GT_UVWR1_U2.data[3],
                2,
                Number((value.onlive_freq.value / 10000).toFixed(1))
              );
              Vue.set(
            ...
            case "轮播":
              Vue.set(
                vm.GT_UVWR1_U1.data[4],
                2,
                Number((value.onlive_user_num.value / 10000).toFixed(1))
              );
              Vue.set(
                vm.GT_UVWR1_U2.data[4],
                2,
                Number((value.onlive_freq.value / 10000).toFixed(1))
              );
              Vue.set(
      ...
      try {
        ...
        Vue.set(vm.GT_UVWR1_V1.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_V1.data[0], 2, beforeWeekFormat);
        if (demand_top[0].key) { }
        demand_top.forEach((value, index) => {
          if (index < 15) {
            // console.log(value.key, Number((value.onlive_user_num.value / 10000).toFixed(1)))
            Vue.set(vm.GT_UVWR1_V1.data[15 - index], 0, value.key);
            Vue.set(
              vm.GT_UVWR1_V1.data[15 - index],
              1,
              Number((value.onlive_user_num.value / 10000).toFixed(1))
            );
          }
        ...
        vm.GT_UVWR1_V1.data.forEach((value, index) => {
          if (index > 0) {
            Vue.set(value, 0, '');
            Vue.set(value, 1, 0);
          }
      ...
      try {
        ...
        vm.GT_UVWR1_V2.data = week_time_temp;
      } catch (error) {
        vm.GT_UVWR1_V2.data = []
        console.log(error);
      }
        ...
        local_program.forEach((value, index) => {
          if (index < 5) {
            ...
            Vue.set(
              ...
              2,
              Number((value.onlive_dur.value / 10000 / 60).toFixed(1))
            );
          }
        });
      } catch (error) {
        vm.GT_UVWR1_V3.data.forEach((value, index) => {
          if (index > 0) {
            Vue.set(value, 0, '');
            Vue.set(value, 2, 0);
          }
      ...
      try {
        ...
        Vue.set(vm.GT_UVWR1_W1.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_W1.data[0], 2, beforeWeekFormat);
        Vue.set(vm.GT_UVWR1_W2.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_W2.data[0], 2, beforeWeekFormat);
        Vue.set(vm.GT_UVWR1_W3.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_W3.data[0], 2, beforeWeekFormat);
        if (demand_user_data[0].key) { }
        demand_user_data.forEach((value, index) => {
          if (index < 10) {
            ...
            Vue.set(vm.GT_UVWR1_W1.data[10 - index], 0, value.key);
            Vue.set(
              vm.GT_UVWR1_W1.data[10 - index],
              1,
              Number((value.demand_user_num.value / 10000).toFixed(1))
            );

            Vue.set(vm.GT_UVWR1_W2.data[10 - index], 0, value.key);
            Vue.set(
              vm.GT_UVWR1_W2.data[10 - index],
              1,
              Number((value.demand_freq.value / 10000).toFixed(1))
            );

            Vue.set(vm.GT_UVWR1_W3.data[10 - index], 0, value.key);
            Vue.set(
              vm.GT_UVWR1_W3.data[10 - index],
              1,
              Number((value.demand_dur.value / 10000 / 60).toFixed(1))
            );
          }
        });
      } catch (error) {
        vm.GT_UVWR1_W1.data.forEach((value, index) => {
          Vue.set(value, 0, '');
          Vue.set(value, 1, 0);
        })
        vm.GT_UVWR1_W2.data.forEach((value, index) => {
          Vue.set(value, 0, '');
          Vue.set(value, 1, 0);
        })
        vm.GT_UVWR1_W3.data.forEach((value, index) => {
          Vue.set(value, 0, '');
          Vue.set(value, 1, 0);
        })
        console.log(error);
      ...
      try {
        ...
        Vue.set(vm.GT_UVWR1_X1.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_X1.data[0], 2, beforeWeekFormat);
        Vue.set(vm.GT_UVWR1_X2.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_X2.data[0], 2, beforeWeekFormat);
        if (program_click_data[0].key) { }
        program_click_data.forEach((value, index) => {
          if (index < 10) {
            // console.log('页面点击和播放时长数据', value.key, value.click_user_num.value)
            Vue.set(vm.GT_UVWR1_X1.data[10 - index], 0, value.key);
            Vue.set(
              vm.GT_UVWR1_X1.data[10 - index],
              1,
              Number((value.click_user_num.value / 10000).toFixed(1))
            );
            Vue.set(vm.GT_UVWR1_X2.data[10 - index], 0, value.key);
            Vue.set(
              vm.GT_UVWR1_X2.data[10 - index],
              1,
              Number((value.click_freq.value / 10000).toFixed(1))
            );
          }
        });
      } catch (error) {
        vm.GT_UVWR1_X1.data.forEach((value, index) => {
          Vue.set(value, 0, '');
          Vue.set(value, 1, 0);
        })
        vm.GT_UVWR1_X2.data.forEach((value, index) => {
          Vue.set(value, 0, '');
          Vue.set(value, 1, 0);
        })
        console.log(error);
      ...
      try {
        ...
        Vue.set(vm.GT_UVWR1_X3.data[0], 1, currentWeekFormat);
        Vue.set(vm.GT_UVWR1_X3.data[0], 2, beforeWeekFormat); //
        if (program_play_data[0].key) { }
        ...
        vm.GT_UVWR1_X3.data.forEach((value, index) => {
          Vue.set(value, 0, '');
          Vue.set(value, 1, 0);
        })
        console.log(error);
    ...
    api_data_m3_range(newValue, oldValue) {
      ...
      try {
        ...
        setTimeout(() => {
          ...
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_V1.data[index], 2, 0);
              ...
              demand_top.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  ...
                  Vue.set(
                    vm.GT_UVWR1_V1.data[index],
                    2,
                    (value2.onlive_user_num.value / 10000).toFixed(1)
                  );
                }
              });
            }
      ...
      try {
        //各类型节目点播数据    点播用户数（万户）
        let demand_user_data =
          newValue.data.responses[14].aggregations.program_type.buckets;
        setTimeout(() => {
          let live_temp = vm.GT_UVWR1_W1.data;
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_W1.data[index], 2, 0);
              // console.log("页面点击用户数本周", value)
              demand_user_data.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  // console.log(value2.key, value2.demand_user_num.value)
                  Vue.set(
                    vm.GT_UVWR1_W1.data[index],
                    2,
                    (value2.demand_user_num.value / 10000).toFixed(1)
                  );
                }
              });
            }
        ...
        //点播次数（万次）上周
        setTimeout(() => {
          let live_temp = vm.GT_UVWR1_W2.data;
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_W2.data[index], 2, 0);
              // console.log("页面点击用户数本周", value)
              demand_user_data.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  // console.log(value2.key, value2.demand_freq.value)
                  Vue.set(
                    vm.GT_UVWR1_W2.data[index],
                    2,
        ...
        //点播时长（万小时）上周
        setTimeout(() => {
          let live_temp = vm.GT_UVWR1_W3.data;
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_W3.data[index], 2, 0);
              // console.log("页面点击用户数本周", value)
              demand_user_data.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  // console.log(value2.key, value2.demand_dur.value)
                  Vue.set(
                    vm.GT_UVWR1_W3.data[index],
                    2,
                    (value2.demand_dur.value / 10000 / 60).toFixed(1)
                  );
                }
              });
            }
      ...
      try {
        //主要栏目点击和播放数据
        //页面点击用户数
        let program_click_data =
          newValue.data.responses[15].aggregations.ti.buckets;
        setTimeout(() => {
          let live_temp = vm.GT_UVWR1_X1.data;
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_X1.data[index], 2, 0);
              // console.log("页面点击用户数本周--", value)
              program_click_data.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  // console.log(value2.key, value2.click_user_num.value)
                  Vue.set(
                    vm.GT_UVWR1_X1.data[index],
                    2,
                    Number((value2.click_user_num.value / 10000).toFixed(1))
                  );
                }
              });
            }
        ...
        setTimeout(() => {
          let live_temp = vm.GT_UVWR1_X2.data;
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_X2.data[index], 2, 0);
              // console.log("页面点击次数本周--", value)
              program_click_data.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  // console.log(value2.key, value2.click_freq.value)
                  Vue.set(
                    vm.GT_UVWR1_X2.data[index],
                    2,
                    Number((value2.click_freq.value / 10000).toFixed(1))
                  );
                }
              });
            }
      ...
      try {
        ...
        setTimeout(() => {
          let live_temp = vm.GT_UVWR1_X3.data;
          live_temp.forEach((value, index) => {
            if (index > 0) {
              Vue.set(vm.GT_UVWR1_X3.data[index], 2, 0);
              // console.log("页面播放时长本周--", value)
              program_play_data.forEach((value2, index2) => {
                if (value2.key == value[0]) {
                  // console.log(value2.key, value2.demand_dur.value)
                  Vue.set(
                    vm.GT_UVWR1_X3.data[index],
                    2,
                    Number((value2.demand_dur.value / 10000 / 60).toFixed(1))
                  );
                }
              });
            }
  ...
  methods: {
    Excel_data_manage() {
      console.log("Excel_data_manage - 5 - m3");
      let vm = this;
      setTimeout(function () {
        if (vm.PR_Report_index == 5) {
          ...
          // temp_DataArr.push([
          //   ["运营商", "移动", "联通", "电信555555555555555.333"],
          //   ["平均", 1, 2, 3]
          // ]);
          // temp_DataArr.push([
          //   ["运营商", "移动", "联通", "电信", "测试22"],
...
<style >
.UVWR_m3 .analysis_of_viewing1 span {
  font-size: 12px;
  color: red;
}
</style>
