    ...
    api_data3(newValue, oldValue) {
      ...
      try {
        ...
        inTheNetworkData.forEach((value, index) => {
          ...
          // console.log("--------------------")
          let register_num = Number(
            (value.register_num.value / 10000).toFixed(2)
          );
          let watch_dur_family = (
            value.watch_dur.value /
            60 /
            value.watch_user_num.value
          ).toFixed(2);
          let cum_paid_rate =
            Number(
              (
                (value.cum_paid_num.value / value.register_num.value) *
                100
              ).toFixed(2)
            ) + "%";
          if (value.key == "active_user") {
            // console.log(value)
            active_user_temp.push(
              register_num,
              watch_dur_family,
              cum_paid_rate
            );
        ...
        //总的订购用户占比
        let total_cum_paid_rate = Number(
          ((total_cum_paid_num / total_register_num) * 100).toFixed(2)
        );
        //总数据total_user_data
